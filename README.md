...
We also release the datasets used in our paper on 🤗 [Hugging Face](https://huggingface.co/datasets/JJHE/FEAT/). Samples for ALDP, LJ-79/128 in these datasets have been pre-aligned to reference configurations to facilitate mini-batch optimal transport (OT) pairing during training. In the dataset, each target has two files, ```*_0*``` and ```_1*```, which represent their environment (0 is vacuum, 1 is implicit solvent).

For custom datasets, especially for larger systems (ALDP, LJ-128, A4, Chig, or larger), you may need to align your configurations first. See the alignment scripts in the `data/` folder (`align_rotation.py` and `align_rotation_permute.py`) and the [data folder README](data/readme.md) for more details.
